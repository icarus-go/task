...
func (t *Task) Start() error {
	...
	if t.state != constant.TASK_STATE_STOP {
		//运行中的任务，直接返回
		return nil
	}

	go func(tt *Task) {
		tt.state = constant.TASK_STATE_WAIT
		...
		for {
			tt.state = constant.TASK_STATE_RUN
			...
			tt.state = constant.TASK_STATE_SLEEP
			select {
			case <-tt.stopChan:
				goto TaskStop
			case <-time.After(i):
	...
	TaskStop:
		t.state = constant.TASK_STATE_STOP
	}(t)
...
// Stop 中止任务
func (t *Task) Stop() {
	if t.state == constant.TASK_STATE_STOPPING || t.state == constant.TASK_STATE_STOP {
		return
	}
	t.state = constant.TASK_STATE_STOPPING
...
// Wait 等待任务中止完成
func (t *Task) Wait() {
	for t.state != constant.TASK_STATE_STOP {
...
// State 当前任务状态
func (t *Task) State() constant.TaskState {
	return t.state
}
